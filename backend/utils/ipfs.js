...
async function initIPFS() {
    ...
    if (IPFS_PROVIDER === 'pinata') {
        if (!PINATA_API_KEY || !PINATA_SECRET_KEY) {
            console.warn('⚠️  Pinata keys missing, falling back to local storage');
            return false;
        }
...
async function uploadToIPFS(data) {
    ...
    if (USE_IPFS) {
        // Try Pinata
        if (IPFS_PROVIDER === 'pinata' && PINATA_API_KEY) {
            try {
...
function isIPFSAvailable() {
    ...
    if (IPFS_PROVIDER === 'pinata' && PINATA_API_KEY) return true;
